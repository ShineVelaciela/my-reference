...
gulp.task('scripts:app', function() {
    ...
    return gulp.src(source.scripts)
        ...
        .pipe($.concat( 'app.js' ))
        .pipe($.ngAnnotate())       //自动填充angularjs依赖注入
        .on('error', handleError)
...
gulp.task('rev:manifest',function () {
    log('manifest scripts..');
    // Minify and copy all JavaScript (except vendor scripts)
    return gulp.src([build.scripts+'/app.js',build.scripts+'/base.js'])
        .pipe($.uglify())           //新增丑化压缩
        .pipe(rev())                  //- 文件名加MD5后缀
        .pipe(gulp.dest(build.scripts))
...
gulp.task('watch', function() {
  ...
  gulp.watch(source.templates.index, ['templates:index']);
    // gulp.watch('./rev/**.json', ['rev']);        //同步监听版本号变化
