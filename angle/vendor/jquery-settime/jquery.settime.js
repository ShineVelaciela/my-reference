...
(function($){
	...
	document.writeln('<style>'+
						'.set-time-'+class_num+'{border:5px solid #a8b2ce;background:#eff8fd;z-index:2000;overflow:hidden;max-width:530px;_width:530px;border-radius:5px;}' +
						...
						'.set-time-'+class_num+' .set-time-'+class_num+'-title{padding:5px;background:#58b3db;overflow:hidden;*zoom:1;}' +
						...
						'.set-time-'+class_num+' td{border:1px solid #bdd7f8;padding:2px;text-align:center;cursor:pointer;font:12px "宋体"} ' +
						'.set-time-'+class_num+' td.unuse{color:#ccc;cursor:wait;} ' +
						'.set-time-'+class_num+' td.use:hover{color:#000;background:#deeefd;} ' +
						'.set-time-'+class_num+' th{padding:2px;background:#d5e5f5;border:1px solid #bdd7f8;}' +
						'.set-time-'+class_num+' .now{background:#eee;color:red;}' +
						'.set-time-'+class_num+' .default{background:#58b3db;color:#555;}' +
						'.set-time-'+class_num+' .selected{background:#3572b0;color:#eee;}' +
	...
	$.fn.extend({
		setTime : function(options){
			...
			self.attr('readonly','true').bind('click', function(e){
				...
				'<div class="set-time-'+class_num+'-title" id="set-time-title-'+id_num+'"><div><b>选择时长</b></div><div class="set-time-'+class_num+'-close"><span><a href="javascript:" title="关闭" id="set-time-'+id_num+'-close">&nbsp;×&nbsp;</a></span></div></div>' +
				...
				thisHtmlObj.css({display:'block', left:(e.pageX > 530 ? e.pageX-500 :e.pageX-20), top:e.pageY+10});
